...
final class TranscriptQueryTests: XCTestCase {
    ...
    func testSearchValidatesInputsBeforeDatabaseRead() throws {
        ...
        XCTAssertThrowsError(try fixture.query.search(phrase: "alpha", contextSegments: -1)) { error in
    ...
    func testContextAtTimelineBoundariesDoesNotUnderflowOrOverflow() throws {
        ...
        let first = try XCTUnwrap(try fixture.query.search(phrase: "cold open", limit: 5, contextSegments: 2).first)
        XCTAssertEqual(first.context.map { $0.identity.sequence }, [0, 1, 2])
        XCTAssertEqual(first.context.map(\ .role), [.match, .after, .after])

        let last = try XCTUnwrap(try fixture.query.search(phrase: "wrap up", limit: 5, contextSegments: 2).first)
        ...
        try writer.persistTimeline(
            IngestChunkTimeline(
                ...
                segments: [
                    ...
                    segment(1, "host", 1.0, 2.0, "Alpha beta alpha beta.", words: ["Alpha", "beta", "alpha", "beta"]),
                    segment(2, "guest", 2.0, 3.0, "middle context only"),
                    segment(3, "producer", 3.0, 4.0, "quoted \"promo\" break then wrap up")
                ],
                speakerTurns: [
                    SpeakerTurnDraft(speakerLabel: "host", startSeconds: 0, endSeconds: 2, confidence: 0.91),
                    SpeakerTurnDraft(speakerLabel: "guest", startSeconds: 2, endSeconds: 3, confidence: 0.84),
                    SpeakerTurnDraft(speakerLabel: "producer", startSeconds: 3, endSeconds: 4, confidence: 0.82)
                ],
        ...
        try writer.persistTimeline(
            IngestChunkTimeline(
                ...
                segments: [
                    ...
                    segment(1, "caller", 1.0, 2.0, "callers say ALPHA BETA", words: ["callers", "say", "ALPHA", "BETA"])
                ],
                speakerTurns: [
                    SpeakerTurnDraft(speakerLabel: "dj", startSeconds: 0, endSeconds: 1, confidence: 0.75),
                    SpeakerTurnDraft(speakerLabel: "caller", startSeconds: 1, endSeconds: 2, confidence: 0.87)
                ],
