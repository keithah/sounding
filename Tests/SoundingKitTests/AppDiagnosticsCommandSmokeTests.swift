...
final class AppDiagnosticsCommandSmokeTests: XCTestCase {
    ...
    func testEvidenceReviewRendersTypedPassWarnAndFailEvidenceWithoutPathLeakage() throws {
        ...
        XCTAssertEqual(result.exitCode, 0, result.diagnosticSummary)
        let output = result.stdoutText
        XCTAssertEqual(lines(in: output, containing: "app-diagnostics evidence=[redacted-path]"), 3, output)
        XCTAssertTrue(output.contains("schemaVersion=1 runID=fixture-pass status=pass"), output)
        XCTAssertTrue(output.contains("schemaVersion=1 runID=live-warn-[redacted-secret] status=warn"), output)
        XCTAssertTrue(output.contains("schemaVersion=1 runID=fixture-fail-[redacted-secret] status=fail"), output)
        ...
        XCTAssertTrue(output.contains("check=playback_muted phase=playback_control required=true status=fail"), output)
        XCTAssertTrue(output.contains("warnings"), output)
        XCTAssertTrue(output.contains("check=live_transcript_observed phase=live_transcript required=false status=warn"), output)
        ...
        XCTAssertTrue(output.contains("kind=stream-source path=https://example.test/live.m3u8"), output)
    ...
    func testMalformedEvidenceFailsWithControlledPathFreeError() throws {
        let directory = try makeTemporaryDirectory(name: "evidence-malformed-user:pass-token=synthetic-secret#frag")
        defer { try? FileManager.default.removeItem(at: directory) }
        let evidenceURL = directory.appendingPathComponent("private-evidence-token=synthetic-secret.json")
        try #"{ "schemaVersion": 1, "runID": "token=synthetic-secret" "#.write(to: evidenceURL, atomically: true, encoding: .utf8)
        ...
        XCTAssertTrue(result.stderrText.contains("app-diagnostics evidence=[redacted-path] error=malformed-app-verify-evidence"), result.stderrText)
        XCTAssertFalse(result.stderrText.contains("Foundation"), result.stderrText)
        assertSanitized(result.stdoutText + result.stderrText, forbiddenLiterals: [
            directory.path,
            evidenceURL.path,
            "user:pass",
            "synthetic-secret",
    ...
    func testUnreadableEvidenceFailsWithControlledPathFreeError() throws {
        let directory = try makeTemporaryDirectory(name: "evidence-missing-user:pass-token=synthetic-secret#frag")
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        XCTAssertNotEqual(result.exitCode, 0, result.diagnosticSummary)
        XCTAssertTrue(result.stderrText.contains("app-diagnostics evidence=[redacted-path] error=unreadable"), result.stderrText)
    ...
    private func makeEvidenceReviewFixture() throws -> EvidenceReviewFixture {
        let directory = try Self.makeTemporaryDirectory(name: "app-diagnostics-evidence-user:pass-token=synthetic-secret#frag")
        let emptyLogDirectory = directory.appendingPathComponent("empty-logs", isDirectory: true)
        try FileManager.default.createDirectory(at: emptyLogDirectory, withIntermediateDirectories: true)
        ...
        let secretSource = "https://user:pass@example.test/live.m3u8?token=synthetic-secret#private-fragment"
        let secretPath = directory.appendingPathComponent("runtime-events-token=synthetic-secret.jsonl").path
        ...
        try evidenceJSON(
            ...
            warnings: 0,
            checks: [
                checkJSON(name: "fixture_source_created", status: "pass", required: true, phase: "fixture"),
                checkJSON(
                    name: "decode_completed",
                    status: "pass",
                    required: true,
                    phase: "decode",
                    facts: #""facts":{"phase":"decode","processedChunks":2,"decodedChunks":2,"scheduledBuffers":0,"diagnosticCount":1,"recentDiagnosticEvents":["decode.completed"],"timelineSnapshotFields":{}}"#
                ),
            ],
            artifacts: []
        ...
        try evidenceJSON(
            ...
            warnings: 1,
            checks: [
                checkJSON(name: "live_stream_registered", status: "pass", required: true, phase: "live_registration"),
                checkJSON(
                    ...
                    liveFacts: liveFactsJSON(secretSource: secretSource, event: "live.transcript.timeout")
                ),
            ],
        ...
        try evidenceJSON(
            ...
            warnings: 0,
            checks: [
                checkJSON(name: "runtime_started", status: "pass", required: true, phase: "runtime_start"),
                checkJSON(
                    name: "playback_muted",
                    status: "fail",
                    required: true,
    ...
    private func evidenceJSON(
        ...
        let metadataJSON = metadata.map { #""\#($0.key)":"\#($0.value)""# }.sorted().joined(separator: ",")
        return """
        {
          "schemaVersion": 1,
    ...
    private func checkJSON(
        ...
        return #"{"name":"\#(name)","status":"\#(status)","required":\#(required),"phase":"\#(phase)"\#(reasonField)\#(factsField)\#(controlFactsField)\#(liveFactsField),"artifacts":[\#(artifacts.joined(separator: ","))]}"#
    }
