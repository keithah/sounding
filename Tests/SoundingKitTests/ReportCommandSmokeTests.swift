...
final class ReportCommandSmokeTests: XCTestCase {
  ...
  func testSeededDatabaseReportsKnownRepeatsAndAdsThroughCLIWithFilters() throws {
    ...
    XCTAssertTrue(
      repeatsHumanText.contains("song=Repeat Artist — Echo Song"), repeatsHuman.diagnosticSummary)
    ...
    XCTAssertEqual(
      repeatGroup.groupKey, "artist-title:repeat artist:echo song", repeatsJSON.diagnosticSummary)
  ...
  private func seedReportFixture(at dbURL: URL) throws -> SeededReportFixture {
    ...
    return SeededReportFixture(hlsStreamID: hlsStreamID)
  }
